...
export const signup = async(req,res,next)=>{
    ...
    res.status(201).json("User created Successfully");
        ...
        next(error);
       // next(errorHandler(500,'error msg'));
    }
...
export const signin = async(req, res, next)=>{
    ...
    try {
        ...
        //destructuring validuser object to hide password
        const {password:pass, ...restInfo} = validUser._doc
        //sending userInfo and storing it in cookie using jwt
...
try {
    ...
    if(user){
       ...
       // const uniqueUser = req.body.name.split(" ").join("").toLowerCase()+Math.random().toString(36).slice(-4);
